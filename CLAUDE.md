...
- gen_tiles.py SPHERE_ROWS 업데이트 필요 (100대 테스트 때) - 아래 번호 규칙대로 순서 작성

### deviceId 번호 규칙 (확정)
- 경도 0°, 위도 +64.29°(가장 위쪽 행)를 1번으로 시작
- 각 행 안에서는 경도 0°부터 시작해 경도가 증가하는 방향(동쪽)으로 번호 증가
- 행 순서는 위도가 높은 쪽(+64.29°)에서 낮은 쪽(−77.14°)으로 내려가며 진행 - 즉 +/− 구분 없이
  위도 값이 큰 행부터 순서대로 번호를 이어붙임(+64.29 → +51.43 → ... → 0 → ... → −64.29 → −77.14)
- gen_tiles.py 기본 동작(스태거 미사용, `--stagger` 안 줌)과 일치 - 홀수 행을 경도로 절반 어긋나게
  배치하는 지그재그(`--stagger`)는 쓰지 않음
...
- [x] Phase 3: FFmpeg 파이프라인 + 신규 기능
        ...
        [x] 100대 확장 테스트
             ...
             네트워크/동기화 안정성 재검증
        [x] 텍스트 스크롤
              서버 대시보드에서 텍스트 입력 → 폰들이 텍스트가 흐름
              전체 방송(파라미터만) + 각 폰이 로컬 렌더링
              100대 평면(row/col) + 439대 구체(lat/lon 기반) 둘 다 지원
              (CLAUDE.md 설계 노트 참고)
...
- **폰 간격(gapRatio) 보정**: 폰 화면(width/height)만으로 캔버스를 이어붙이면 실제
  ...
  - **구체(sphere) 지원 (439대, 2026-08)**: 위도마다 폰 개수/간격이 달라(9~50대)
    "피치 하나"로 안 잡혀서, flat의 매니페스트 전역 `gap` 대신 **행마다 다른 값**을
    `gen_sphere()`가 직접 계산해 타일 `meta`에 저장한다(`gapRatioX`=
    `1 - d_lon/(360/count)`, `gapRatioY`=`1 - d_lat/행간위도차` - 행간위도차는 중간
    행이면 위아래 이웃 위도차의 평균, 맨 위/맨 아래 행은 이웃이 한쪽뿐이라 그 거리를
    그대로 씀). `gen_configs.py`는 `meta`에 `gapRatioX`/`gapRatioY`가 있으면(sphere)
    그걸 우선 쓰고, 없으면(flat) 기존 매니페스트 전역값을 쓴다.

### 텍스트 스크롤 구체(sphere) 가로축 재설계 - 경도 기반 (2026-08)
- 439대로 확장하면서 텍스트 스크롤이 깨짐 - 원인 두 가지:
  1. `gen_sphere()`가 만드는 타일 meta엔 `row/lat/lon/d_lat/d_lon`만 있고 `col`이
     없어서, 안드로이드의 `myCol = config?.col ?: 0`이 전부 0이 됨 → 439대가 전부
     캔버스의 같은 조각만 보여줌.
  2. 설령 `col`을 채워도, `TextScrollView.kt`의 `gridCols = rowCounts.maxOrNull()`가
     "가장 넓은 행 기준 균일 격자"를 전제해서 행마다 대수가 다른(9~50대) 구체엔
     안 맞음 - 100대 평면(전부 20대 균일)에서만 우연히 문제없었던 것.
- **`col`은 추가하지 않기로 함**: 코드 전체에서 sphere 타일의 `col`을 참조하는 곳이
  `server.js`의 `/api/text-pattern/start`(텍스트 패턴 발행 대상 판정) 딱 한 곳
  있었는데, 지금은 `col`이 없어서 그 폰들이 "발행 대상에서 스킵"되는 안전한 상태다
  (텍스트 패턴은 sphere 미지원이라는 기존 결정과 일치). `col`을 채우면 이 스킵이
  풀리면서 텍스트 패턴에도 위와 같은 균일 격자 버그가 새로 생기므로, 안 쓰는 필드를
  습관적으로 추가하지 않는다는 원칙대로 넣지 않음.
- **해결**: 열 인덱스 대신 **경도(lon) 자체**를 가로 위치 기준으로 씀 -
  `위치 비율 = lon / 360°`. 행마다 대수가 달라도 항상 같은 각도 기준으로 정렬되고,
  `gridWidthPx`(전체 캔버스 폭, 가장 넓은 행 기준 - 기존 계산 그대로 재사용)에
  이 비율을 곱해서 전역 캔버스 좌표를 구한다. flat/sphere 분기는 새 필드
  없이 `config.lon`의 유무로 판단(`myLon`이 null이면 flat, 값 있으면 sphere) -
  flat 경로는 구조적으로 항상 이 값이 null이라 회귀 위험이 없다. 세로축(위도)은
  439대 배치가 위도 간격 12.857°로 균일해서 flat과 같은 `myRow`/`totalRows`
  방식을 그대로 재사용.
